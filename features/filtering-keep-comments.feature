Feature: Keeping comments in the result of gff3-ffetch tool

  The goal of this feature is to let the users
  keep lines with comments in the output of gff3-ffetch.

  Scenario: keep comments while filtering a file
    Given I have a GFF3 file
    When I set up the filter to leave most records after filtering
    And set the output to be a string
    And set the keep_comments option
    And run the filter
    Then I should receive a string with 16 lines
    And the 10th line should start with "#."

  Scenario: keep comments while filtering a string
    ...
    Then I should receive a string with 8 lines
    And the 5th line should start with "#."
