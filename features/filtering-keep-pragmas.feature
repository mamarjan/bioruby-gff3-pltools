Feature: Keeping pragmas in the result of gff3-ffetch tool

  The goal of this feature is to let the users
  keep lines with pragmas in the output of gff3-ffetch.

  Scenario: keep pragmas while filtering a file
    Given I have a GFF3 file
    When I set up the filter to leave most records after filtering
    And set the output to be a string
    And set the keep_pragmas option
    And run the filter
    Then I should receive a string with 16 lines
    And the first line should contain "##gff-version 3"

  Scenario: keep pragmas while filtering a string
